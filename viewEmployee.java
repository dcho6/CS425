...
public class viewEmployee extends JFrame
{
	...
	public viewEmployee() {
		...
		employeeTable.setModel(new DefaultTableModel(
			new Object[][] {},
			new String[] {
				"SSN", "Emp ID", "F Name", "L Name", "Username", "Password", "Privilege", "Pay Type", "Pay"
			}
		) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					for(int i=0; i<detail.size();i++) {
						...
						row[2] = detail.get(i).getFirstName();
						
						row[3] = detail.get(i).getLastName();
						
						row[4] = detail.get(i).getUsername();
						
						row[5] = detail.get(i).getPassword();
						
						row[6] = detail.get(i).getPrivilege();
						
						row[7] = detail.get(i).getPayType();
						
						row[8] = detail.get(i).getPay();
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {/*this is the search specific function*/
					...
					ArrayList<Employee> detail = functions.employeeSearchView();
